...
export function findEditableFromTarget(target: EventTarget | null): EditableElement | null {
...
export function buildDictationAnchor(element: EditableElement): DictationAnchor {
  if (element instanceof HTMLInputElement || element instanceof HTMLTextAreaElement) {
...
export function setEditableText(element: EditableElement, value: string, caretPos?: number): void {
  if (element instanceof HTMLInputElement || element instanceof HTMLTextAreaElement) {
    element.value = value;
...
function getContentEditableSelectionRange(element: HTMLElement): Range {
  const selection = window.getSelection();
  if (selection && selection.rangeCount > 0) {
    const range = selection.getRangeAt(0);
    if (element.contains(range.startContainer) && element.contains(range.endContainer)) {
...
export function insertEditableText(element: EditableElement, text: string): void {
  if (!text) {
    return;
  }
...
export function getEditableLabel(element: EditableElement): string {
  if (element instanceof HTMLInputElement || element instanceof HTMLTextAreaElement) {
